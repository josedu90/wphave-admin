	...
	class wp_less {
		...
		public function parse_stylesheet( $src, $handle ) {
			...
			try {
				...
				echo '<div style="background: #f1d298; color: #6f4314; padding: 5px 10px; font-size: 15px;">' . __('A critical error occurred while compiling the css files', 'wphave-admin') . ': ' . $ex->getMessage() . '</div>';
		...
		public function update_cached_file_data( $path, $file_data ) {
			...
			update_option( 'wp_less_cached_files', $caches, 'yes' );
		}
